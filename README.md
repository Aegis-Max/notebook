...
- 首次运行包含该示例的版本时，会向笔记库一次性加入一篇可编辑、可删除的经典康奈尔笔记；已有笔记保持不变，删除示例后不会复活。
...
- **模型 API 设置**：通过顶部工具菜单或 `Command/Ctrl + ,` 打开，支持 OpenAI、DeepSeek、自定义 OpenAI 兼容接口及本机 Ollama；可检测、搜索或手填模型，并在保存前测试当前草稿连接。
...
- “检测模型”和“测试草稿连接”不会保存配置，也不会发送笔记内容；只有点击“保存设置”才会原子写入当前配置与可选新密钥。已保存密钥与规范化后的 API 基础地址绑定，更换地址后必须提供对应的新密钥。
- API 地址按“基础地址”处理：应用只在末尾追加 `/models` 和 `/chat/completions`，不会自动添加或删除 `/v1`；设置页会预览最终请求地址。
