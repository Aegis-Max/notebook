...
} from '../src/domain/builtin-example.js';
import { validateNote } from '../src/domain/note-store.js';

const EXPECTED_CUES = `基础篇
...
const EXPECTED_NOTES = `基础篇｜上手实战、核心配置与注意事项
01:01 下载并安装 Codex
01:25 Codex 登录与套餐选择
03:27 实现第一版马克笔记
06:05 预览区域的使用
...
test('创建内容完整且可持久化的经典 Cornell 内置示例', () => {
  const note = createBuiltinExampleNote();
  ...
  assert.equal(BUILTIN_EXAMPLE_NOTE_ID, note.id);
  assert.equal(validateNote(note).valid, true);
  assert.equal(Object.getPrototypeOf(note), Object.prototype);
});

test('每次调用都返回内容相同但相互独立的普通 Note 对象', () => {
  const first = createBuiltinExampleNote();
  const second = createBuiltinExampleNote();

  assert.notEqual(first, second);
  assert.deepEqual(first, second);

  first.title = '用户修改后的标题';
  first.notes = '用户修改后的笔记';

  assert.equal(second.title, 'Codex 工作坊：从基础使用到插件、技能与自动化');
  assert.equal(second.notes, EXPECTED_NOTES);
});
